    ...
    IERC721Enumerable
{
    ...
    uint256 private _nblocks_active = 1;

    /**
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    constructor(string memory name, string memory symbol, uint256 nblocks_active) public {
        _name = name;
        _symbol = symbol;
        _nblocks_active = nblocks_active;
    ...
    function setNBlocksActive(uint256 nblocks_active) public {
        //require(isAdmin());
        _nblocks_active = nblocks_active;
    }

    function nBlocksActive() public view returns (uint256) {
        return _nblocks_active;
    }

    function activeToken() public view returns (uint256) {
        require(
            totalSupply() > 0,
            "totalSupply must be > 0"
        );
        uint256 index = (block.number.mod(totalSupply().mul(_nblocks_active))).div(_nblocks_active);
